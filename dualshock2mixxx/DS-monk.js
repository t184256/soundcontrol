//
// DS PS3 Game Controller HID mapping
// Copyright (C) 2012, Ilkka Tuohela
// Copyright (C) 2014, Alexander Sosedkin
...
// DSController is defined in DS.hid.js
DS = new DSController();

// Mandatory function for mixxx controllers
DS.init = function(id) {
    DS.id = id;
    var controller = DS.controller;
    DS.registerInputPackets();
    DS.registerCallbacks();
    ...
    controller.startAutoRepeatTimer = function(timer_id,interval) {
        ...
        controller.timers[timer_id] = engine.beginTimer(
            interval,
            "DS.controller.autorepeatTimer()"
        )
    }
    HIDDebug("DS controller initialized: " + DS.id);
}

// Mandatory function for mixxx controllers
DS.shutdown = function() {
    DS.controller.close();
    HIDDebug("DS controller shutdown: " + DS.id);
}

// Mandatory function to receive anything from HID
DS.incomingData = function(data,length) {
    DS.controller.parsePacket(data,length);
}

// Register callbacks for "hid" controls defined in DSController
DS.registerCallbacks = function(id) {
    var controller = DS.controller;
    var packet = DS.controller.getInputPacket("control");
    if (packet==undefined) {
    ...
    controller.setCallback("control","hid","button_bottom_left",DS.front_left);
    controller.setCallback("control","hid","button_top_left",DS.front_left);
    controller.setCallback("control","hid","button_bottom_right",DS.front_right);
    controller.setCallback("control","hid","button_top_right",DS.front_right);

    // Callbacks for pressure sensitive buttons front of controller
    controller.setCallback("control","hid","pressure_bottom_left",DS.left_bend);
    controller.setCallback("control","hid","pressure_top_left",DS.left_bend);
    controller.setCallback("control","hid","pressure_bottom_right",DS.right_bend);
    controller.setCallback("control","hid","pressure_top_right",DS.right_bend);

    // Callbacks for jog controls
    controller.setCallback("control","hid","jog_left_y",DS.left_jog);
    controller.setCallback("control","hid","jog_right_y",DS.right_jog);

    controller.setCallback("control","hid","jog_left_x",DS.left_jog);
    controller.setCallback("control","hid","jog_right_x",DS.right_jog);

};

DS.front_left = function(field) {
    var controller = DS.controller;
